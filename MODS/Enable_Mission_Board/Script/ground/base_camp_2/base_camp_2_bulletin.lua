...
function base_camp_2_bulletin.Mission_Board_Action(obj, activator)
    ...
    if SV.MissionPrereq.NumDungeonsCompleted >= dungeons_needed then
        ...
        UI:ResetSpeaker()
        UI:WaitShowDialogue(Text.FormatKey('Mission_Board_Locked'))
        UI:WaitShowDialogue(Text.FormatKey('Mission_Board_Locked_2', dungeons_needed - SV.MissionPrereq.NumDungeonsCompleted))
    end
...
function base_camp_2_bulletin.Outlaw_Job_Clear(job)
    ...
    if job.Client == 'magna' then
        ...
        UI:WaitShowDialogue(Text.FormatKey('Outlaw_Capture_Cutscene_001', _DATA:GetMonster(outlaw.CurrentForm.Species):GetColoredName()))

        GAME:WaitFrames(20)
        UI:WaitShowDialogue(Text.FormatKey('Outlaw_Capture_Cutscene_002'))
        GAME:WaitFrames(20)
        ...
        if job.BonusReward ~= '' then
            UI:SetSpeaker(magna)
            GAME:WaitFrames(20)
            UI:WaitShowDialogue(Text.FormatKey('Outlaw_Capture_Cutscene_003'))
            GAME:WaitFrames(20)
        ...
        UI:SetSpeaker(magna)
        UI:WaitShowDialogue(Text.FormatKey('Outlaw_Capture_Cutscene_004'))
        ...
        UI:WaitShowDialogue(Text.FormatKey('Outlaw_Retrieve_Cutscene', item:GetDisplayName()))
        GAME:WaitFrames(20)
        UI:WaitShowDialogue(Text.FormatKey('Mission_Generic_Reward'))
        GAME:WaitFrames(20)
        ...
        if job.BonusReward ~= '' then
            UI:SetSpeaker(client)
            GAME:WaitFrames(20)
            UI:WaitShowDialogue(Text.FormatKey('Mission_Generic_Reward_2'))
            GAME:WaitFrames(20)
            COMMON.RewardItem(job.BonusReward)
        end
...
function base_camp_2_bulletin.Mission_Job_Clear(job)
    ...
    if job.Client == job.Target and job.Type ~= COMMON.MISSION_TYPE_ESCORT then
        ...
        if job.Type == COMMON.MISSION_TYPE_RESCUE then
            UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Rescue'))
        elseif job.Type == COMMON.MISSION_TYPE_EXPLORATION then
            ...
            UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Exploration', zone:GetColoredName()))
        elseif job.Type == COMMON.MISSION_TYPE_LOST_ITEM then
            local item = RogueEssence.Dungeon.InvItem(job.Item)
            UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Lost_Item', item:GetDisplayName()))
        else--delivery 
            local item = RogueEssence.Dungeon.InvItem(job.Item)
            UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Delivery_Item', item:GetDisplayName()))
        end
        ...
        if job.Type == COMMON.MISSION_TYPE_ESCORT then
            UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Escort'))
        else
            if job.Special == MISSION_GEN.SPECIAL_CLIENT_LOVER then
                UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Lover'))
            elseif job.Special == MISSION_GEN.SPECIAL_CLIENT_RIVAL then
                UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Rival'))
            elseif job.Special == MISSION_GEN.SPECIAL_CLIENT_CHILD then
                UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Child'))
            else
                UI:WaitShowDialogue(Text.FormatKey('Mission_Response_Rescue_Friend'))
            end
        end
        GAME:WaitFrames(20)
        UI:WaitShowDialogue(Text.FormatKey('Mission_Generic_Reward'))
...
function base_camp_2_bulletin.RewardEXP(job)
    ...
    UI:ResetSpeaker()
    UI:WaitShowDialogue(Text.FormatKey('Mission_Handout_EXP', exp_reward_string))
